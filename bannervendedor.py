...
class BannerVendedor(FloatLayout):
    def __init__(self,**kwargs):
        ...
        link = f'https://aplicativovendashash-8570e-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor}"'
        requesicao = requests.get(link)
        requesicao_dic = requesicao.json()
        valor = list(requesicao_dic.values())[0]
        avatar = valor['avatar']
        ...
        imagem = ImageButton(source=f'icones/fotos_perfil/{avatar}',pos_hint={'right':0.4,'top':0.9}, 
                                    size_hint=(0.3,0.8),on_release=partial(my_app.carregar_vendas_vendedor,valor))
        label_id = LabelButton(text=f'ID Vendedor: {id_vendedor}',pos_hint={'right':0.9,'top':0.9}, 
                                    size_hint=(0.5,0.5),on_release=partial(my_app.carregar_vendas_vendedor,valor))
        label_total = LabelButton(text=f'Total de Vends: {total_vendas}',pos_hint={'right':0.9,'top':0.6}, 
                                    size_hint=(0.5,0.5),on_release=partial(my_app.carregar_vendas_vendedor,valor))
