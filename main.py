...
class MainApp(App):
    ...
    def carregar_perfils_usuario(self):
        for foto in os.listdir('icones/fotos_perfil'):
            perfil = ImageButton(source=os.path.join('icones/fotos_perfil', foto),
        ...
        try:
            ...
            if requesicao_dic == {}:
                ...
                if id_vendedor_adicionado in equipe:
                    ...
                    self.equipe = f"{self.equipe},{id_vendedor_adicionado}"
    ...
    def carregar_scroll_views_venda(self):
        ...
        arquivo2 = os.listdir('icones/fotos_produtos')
        pagina_adicionarvendas = self.root.ids['adicionarvendaspage']
        ...
        for foto_cliente in arquivo:
            imagem = ImageButton(source=os.path.join('icones/fotos_clientes', foto_cliente),
                                 on_release=partial(self.selecionar_cliente, foto_cliente))
            nome_cliente = foto_cliente.replace('.png', '').capitalize()
            label = LabelButton(text=f'{nome_cliente}', on_release=partial(self.selecionar_cliente, foto_cliente))
            lista_clientes.add_widget(imagem)
            lista_clientes.add_widget(label)

        for foto_produto in arquivo2:
            imagem = ImageButton(source=os.path.join('icones/fotos_produtos', foto_produto),
                                 on_release=partial(self.selecionar_produto, foto_produto))
            nome_produto = foto_produto.replace('.png', '').capitalize()
            label = LabelButton(text=f'{nome_produto}', on_release=partial(self.selecionar_produto, foto_produto))
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (
            ...
            foto_produto = f"{produto}.png"
            foto_cliente = f"{cliente}.png"
            ...
            requesicao = requests.get(
                f'https://aplicativovendashash-8570e-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}')
            total_vendas = float(requesicao.json())
            total_vendas += preco
            info = f'{{"total_vendas":"{total_vendas}"}}'
            requests.patch(
                f'https://aplicativovendashash-8570e-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}',
                data=info)
            homepage.ids['label_total_vendas'].text = f'[color=000000]Total de Vendas:[/color] [b]R${total_vendas}[/b]'
    ...
    def carregar_todas_vendas(self):
        ...
        requesicao = requests.get(
            f'https://aplicativovendashash-8570e-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requesicao_dic = requesicao.json()
    ...
    def sair_todas_vendas(self):
        self.root.ids['foto_perfil'].source = os.path.join('icones/fotos_perfil', self.avatar)
        self.mudar_tela('ajustespage')
